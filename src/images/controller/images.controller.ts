...
export class ImageController {
    ...
    create(@Body() imageDto: ImageDTO, @Res() response) {
        this.imageService.createImage(imageDto).then( image => {
            response.status(HttpStatus.CREATED).json(image);
        }).catch( () => {
            response.status(HttpStatus.FORBIDDEN).json({message:'error in image creation'});
        });
    }
    ...
    getAll(@Res() response) {
        this.imageService.getAll().then( image => {
            ...
            response.status(HttpStatus.FORBIDDEN).json({message:'error in get all images'});
        });
    }
}
